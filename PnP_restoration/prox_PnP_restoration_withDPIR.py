...
class PnP_restoration():
    ...
    def restore(self, img, init_im, clean_img, degradation,extract_results=False):
        ...
        if self.hparams.PnP_algo == 'DPIR':
            ...
            dpir_model.load_state_dict(torch.load('../DPIR/model_zoo/dncnn3.pth'), strict=True)
            dpir_model.eval()
            for _, v in dpir_model.named_parameters():
                v.requires_grad = False
            dpir_model = dpir_model.to(self.device)
            dpir_sigmas = torch.logspace(torch.log10(49/255), torch.log10(self.hparams.noise_level_img/255), self.hparams.maxitr)
        ...
        while i < self.hparams.maxitr and abs(diff_Psi)/Psi_old > self.hparams.relative_diff_Psi_min and BFGSBreakFlag > 0:
            ...
            if self.hparams.PnP_algo == 'PGD':
                ...
                if i > 0 and torch.abs(phi_x - Psi_list[-1]) < 1e-8:
                    BFGSBreakFlag=False
            elif self.hparams.PnP_algo == 'DPIR':
                # HQS: prox_f follwed by denoiser
                y = self.calculate_prox(x_old) # prox step
